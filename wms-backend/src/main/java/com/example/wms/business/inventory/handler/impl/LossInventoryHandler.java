 ...
 * 报损/报溢库存处理器
 * <p>
 * 报损单审核通过时调用，根据 lossQty 正负决定操作方向：
 * - lossQty > 0（报损）：扣减仓库库存
 * - lossQty < 0（报溢）：增加仓库库存
 * 同时写入库存变动流水(WmsInventoryLog)用于追溯。
 */
@Component
@Order(Ordered.HIGHEST_PRECEDENCE)
@RequiredArgsConstructor
public class LossInventoryHandler implements InventoryChangeHandler {
    ...
    public void handle(Long billId, String billNo, LocalDateTime billTime, List<? extends ChangeItem> items) {
        ...
        for (ChangeItem item : items) {
            ...
            // 报损是扣减，找不到同批次就退化到 仓库+SKU 下可用数量最大的一条，只要总库存够就允许扣
            if (inv == null && qty < 0) {
